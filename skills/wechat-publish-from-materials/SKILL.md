...
description: 当用户想把“个人资料 + 参考内容”整理成一篇适合自己口吻的公众号文章，并在检查后送进微信公众号草稿箱时使用。第一次接入时，只需要用户准备 3 样东西：persona 文件、参考内容、公众号 appid/secret。默认走最稳主链：读取资料 → 生成正文 → 发布前检查 → 推送草稿箱。不要主动暴露飞书预览、图片桥、复杂 workflow 这些进阶能力，除非用户明确需要。
---
